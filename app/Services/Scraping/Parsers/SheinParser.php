...
class SheinParser implements MerchantParser
{
    ...
    {
        return (bool) preg_match('/shein\.(com|co\.uk|fr|de|es|it)$/i', $domain)
            || str_ends_with($domain, '.shein.com');
    }

    private function cleanName(string $name): string
    {
        $name = preg_replace('/\s*[-|]\s*SHEIN.*$/i', '', $name);
        $name = preg_replace('/\s*\|\s*SHEIN.*$/i', '', $name);
    ...
    private function guessCurrencyFromUrl(string $url): string
    {
        ...
        if (preg_match('/shein\.(fr|de|es|it)$/i', $host)) {
            return 'EUR';
        }
        if (preg_match('/\/([a-z]{2})\//', parse_url($url, PHP_URL_PATH) ?? '', $m)) {
            return match ($m[1]) {
    ...
    {
        if (preg_match('/"salePrice"\s*:\s*\{[^}]*"amount"\s*:\s*([0-9.]+)/i', $html, $m)) {
            return $m[1];
        }
        if (preg_match('/"price"\s*:\s*([0-9.]+)/i', $html, $m)) {
            return $m[1];
        }
